...
module.exports = {
  ...
  sendBackError: false, // by default do not send back actual error
  ...
  construct: function(self, options) {
    ...
    self.addRestApiRoutes = function() {
        ...
        return async.series([ distinct, countPieces, findPieces, renderPieces ], function(err) {
          if (err) {
            ...
            return req.res.status(500).send({
              error: options.sendBackError ? err : 'error'
            });
          }
      ...
      // If no error, send pack the piece. Set sendBackError option to
      // true if the actual error message is to be sent back.

      self.respond = function(req, err) {
        if (err) {
          if (err === 'notfound') {
      ...
      self.restPutOrPatch = function(req, id, action) {
        var respond = self.respond.bind(null, req);
        if (action === 'patch') {
